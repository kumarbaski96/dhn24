...
$result = $conn->query("SELECT * FROM news ORDER BY id DESC LIMIT $limit OFFSET $offset");
?>
                ...
                <td>
                    <?php if ($row['status'] == 1) { ?>
                        <a href="update_status.php?id=<?= $row['id']; ?>&status=1"
                           ...
                           onclick="return confirm('Deactivate this news?')">Active</a>
                    <?php } else { ?>
                        <a href="update_status.php?id=<?= $row['id']; ?>&status=0"
                           class="badge bg-danger"
                           onclick="return confirm('Activate this news?')">Inactive</a>
                    <?php } ?>
                </td>
                <td>
